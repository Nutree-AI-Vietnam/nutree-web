...
interface Props {
  ...
  onApprovePinnedReferences: (foodReferenceIds: number[]) => Promise<void>;
}
...
function UnverifiedReferences({ references, onApprove }: { references: MealCatalogUnverifiedReference[]; onApprove: (foodReferenceIds: number[]) => Promise<void> }) { const unique = Array.from(new Map(references.map((reference) => [reference.food_reference_id, reference])).values()); if (!unique.length) return null; return <div className="border border-energy-orange/40 bg-energy-orange-soft p-3"><div className="flex flex-wrap items-start justify-between gap-3"><div><p className="font-bold text-energy-orange">Pinned references need verification <span className="text-muted">({unique.length})</span></p><p className="mt-1 text-sm text-muted">Review the displayed source and nutrition, then verify these exact references. This does not approve the source for other records.</p></div><button type="button" onClick={() => void onApprove(unique.map((reference) => reference.food_reference_id))} className="border border-primary-forest bg-primary-forest px-3 py-2 text-xs font-bold text-white hover:bg-primary-teal">Review, verify &amp; recheck all ({unique.length})</button></div><ul className="mt-3 space-y-1 text-xs text-foreground">{unique.map((reference) => <li key={reference.food_reference_id}>#{reference.food_reference_id} · {reference.food_reference_name} · {reference.source} · used by {references.filter((item) => item.food_reference_id === reference.food_reference_id).length} recipe ingredient{references.filter((item) => item.food_reference_id === reference.food_reference_id).length === 1 ? '' : 's'}</li>)}</ul></div>; }
